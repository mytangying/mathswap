...
import { TokenAmount, Token, ChainId, Percent, JSBI } from 'mathswap-sdk'
...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getEtherscanLink(1, 'abc', 'transaction')).toEqual('https:/cn.etherscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getEtherscanLink(1, 'abc', 'token')).toEqual('https:/cn.etherscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getEtherscanLink(1, 'abc', 'address')).toEqual('https:/cn.etherscan.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getEtherscanLink(2, 'abc', 'address')).toEqual('https:/cn.etherscan.com/address/abc')
    })
    it('ropsten', () => {
